<template>
  <div  id="editor" class="match-parent" style=" height: 100%; padding: 65px 0 0; box-sizing: border-box ; position: relative; ">
    <div style="height: 25px;  position: absolute; top: 0 ; left: 0 ; width: 100%;" >
      <span><i class="el-icon-setting"></i></span>
      <span class="sidelogotxt">&nbsp;&nbsp;全景编辑</span>
    </div>
    <div class="save-view" style="height: 40px; position: absolute; top: 25px ; left: 0 ; width: 100%;">
      <el-badge  v-show="toSaveFlag" is-dot>
        <el-button  size="small" type="primary" icon="el-icon-search" @click="save()"
                   is-dot style="margin-left: 10px;">保存</el-button>
      </el-badge>
      <el-button  size="small" type="primary" v-show="!toSaveFlag" @click="save()"
                 icon="el-icon-search" is-dot style="margin-left: 9px;">保存</el-button>
      <el-button type="primary" size="small"  @click="preview()">预览<i class="el-icon-view"></i></el-button>
    </div>
    <el-menu default-active="2"   class="el-menu-vertical-demo "  style="height: 100%; width: 100%;">
      <el-submenu index="1">
        <template slot="title">
    ...
    <div class="hotspot-detail" v-show="showViewManagementFlag">
      ...
      <div class="hotspot-detail-content" style="height:668px; overflow-y: auto;">
       ...
       <!-- <label class="label" style="display:block; position: relative;">等待时间(秒)
        ...
        <label class="label" style="display:block; position: relative;">速度
          <el-input v-model="autoSpinWaitingTime" class="speed" @change="updatedAutoSpin()"
                    placeholder="请输入内容" :disabled="rotateON"></el-input>
        </label>
    ...
    <el-dialog
      ...
      <span style="color:black">场景名称</span>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      //预览
      preview() {
        window.open('http://192.168.10.34:3333/test/tour.html');
      },
      //隐藏或者显示下方自带控制条
      ...
      /*  if (!this.toSaveFlag) return;
        ...
        let url = UrlManager.saveEdit();
        let formdata = new FormData();
        formdata.append("id",this.panopo.id);
        formdata.append("scenes",JSON.stringify(data));
