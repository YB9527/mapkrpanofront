<template>
  <div id="editor" class="column is-2 match-parent">
    <div class="columns is-gapless match-parent">
      ...
      <div class="column is-10">
        ...
        <div class="view-module has-text-white" v-show="module == 1"
          ...
          <div class="view-content">
            ...
            <label class="label has-text-white">
              速度
              <input class="input is-small" :disabled="!autoSpinFlag" v-model="autoSpinSpeed"
                     @change="updatedAutoSpin()">
            </label>
    ...
    <my-dialog :show="showModifySceneNameFlag" title="场景修改" v-on:close="closeModifySceneName">
      <div class="modify-scene-name">
        <div class="field">
          <span class="panoname" style="color:black">场景名称</span>
          <p class="control">
  ...
  export default {
    ...
    methods: {
      ...
      initView() {
        ...
        this.autoSpinSpeed = this.krpano.get('autorotate.speed')
      ...
      changeScene(scene) {
        ...
        if (currentScene.autorotate) {
          ...
          this.krpano.set('autorotate.speed', currentScene.autorotate.speed)
        }
      ...
      preview() {
        this.$store.commit("getCustom", {
          url: URLManager.findvrurlbyid() + this.panopo.id,
          callback: htmlurl => {
            console.log(htmlurl)
            window.open(htmlurl)
          }
...
<style lang="scss" scoped>
  ...
  .hotspot-detail .hotspot-detail-header a[data-v-47443d5d] {
    margin: 9px;
  }
  ...
  .scene-module .scene-item .scene-name {
    line-height: 36px;
    height: 36px;
    width: 164px;
    padding-left: 5px;
  ...
  .camera .camera-button[data-v-47443d5d] {
    position: absolute;
    left: 42%;
    top: 30%;
  ...
  * {
  ...
  .view-module .view-content label[data-v-47443d5d] {
    font-weight: 100;
    font-size: 15px;
    margin-bottom: 15px;
  }
